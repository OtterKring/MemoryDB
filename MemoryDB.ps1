...
class Index {
    ...
    Index () {}     # for inheritance purpose only
    ...
    # THIS IS >>>THE<<< METHOD to lookup data using this index!
    ...
    [void] AddEntry ( [PSObject]$DataSet ) {
        ...
        if ( [string]::IsNullOrEmpty( $LookupValue ) ) { $LookupValue = '<nokey>' }
...
class UniqueIndex : Index {
    ...
    UniqueIndex () {
        ...
        Write-Warning '[UniqueIndex]::new( [PSObject[]]$Data", [string]$Key )'
        Write-Warning '[UniqueIndex]::new( [PSObject[]]$Data", [string]$Key , [bool]$CaseInsensitiveKey )'
    }

    # Constructor with dataset and key as parameters
...
class MemoryDB {
    ...
    # use the current calue of CaseInsentitiveKeys to determine this index' case sensitivity
    [void] NewIndex ( [string]$KeyString ) {

        $this.NewIndex( $KeyString, $this.CaseInsensitiveKeys )

    }

    # add a new non-unique index to the MemoryDB
    # case sensitivity is explicity defined
    [void] NewIndex ( [string]$KeyString, [bool]$CIK ) {

        if ( $this.Indices -notcontains $KeyString ) {

            $this.IX.Add( [Index]::new( $this.Data, $KeyString, $CIK ) )
